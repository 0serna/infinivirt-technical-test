...
**Comment**:
Message on a Ticket thread. Visibility is `public` (customer-facing) or `internal` (staff only). In the first release, with no Client portal, every authenticated User who can consult the Ticket sees both types. Consult of a Ticket includes the full Comment thread, oldest first — same visibility rules as above. Adding a Comment requires List Scope (same as consult); being Assignee or `created_by` is not an extra gate. Allowed in any Ticket Status. Append-only: body and visibility do not change after create. Creating a Comment refreshes the Ticket's `updated_at`. Agent may create only `public`; Supervisor and Administrator may create `public` or `internal` (cascade). When visibility is omitted on create, it is `public`. A consultable Ticket can still reject a create when the Role may not use the requested visibility — that is Authorization, not “Ticket does not exist”.
_Avoid_: Note, message, reply (as a domain type); treating create like Status Transition Assignee gates; editing or deleting Comments; Comments that leave `updated_at` unchanged; Supervisor-only-internal as if it removed Agent’s `public`; collapsing out-of-scope and forbidden visibility into one outcome; requiring an explicit visibility with no default; omitting the thread from Ticket consult; newest-first as the domain order
